...
public class FocusHandler implements WXCallbackHandler {
    ...
    public void handle(WXCallbackContext context) {
        String content = "鼓掌/恭喜您发现汽车营销行业新大陆——微车数据服务平台！\n\n回复：\n";
        List<Keyword> keywords =  keywordRepository.findAllBySubSend(Keyword.SUBSEND_Y);
        content = keywords.stream()
                .reduce(content
                        ,(item,keyword)->item+=keyword.getKeyword()+"--"+keyword.getDeclare()
                        ,(item1,item2)->item1+"\n"+item2);
        content+="\n" +
                "酷/汽车品牌销量、报价、促销活动、舆情等信息数据，点击菜单“我的”进行订阅和留言！\n" +
                "可爱/有任何需求或者不满，欢迎随时骚扰哦！玫瑰/";
        try {
